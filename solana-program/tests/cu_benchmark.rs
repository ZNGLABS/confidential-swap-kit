...
fn preuve_valide_acceptee_et_cout_mesure() {
    let data = build_instruction_data(TAG_VERIFY, &PROOF_A, &PROOF_B, &PROOF_C, &PUBLIC_INPUTS);
...
fn entree_publique_modifiee_refusee() {
    ...
    pubs[0][31] ^= 1;
    let data = build_instruction_data(TAG_VERIFY, &PROOF_A, &PROOF_B, &PROOF_C, &pubs);
...
fn preuve_modifiee_refusee() {
    let mut a = PROOF_A;
    a[0] ^= 1;
    let data = build_instruction_data(TAG_VERIFY, &a, &PROOF_B, &PROOF_C, &PUBLIC_INPUTS);
    let (refuse, _) = executer(data);
...
fn taille_invalide_refusee() {
    let mut data = build_instruction_data(TAG_VERIFY, &PROOF_A, &PROOF_B, &PROOF_C, &PUBLIC_INPUTS);
    data.pop();
    let (refuse, _) = executer(data);
...
#[test]
fn tag_inconnu_refuse() {
    let data = build_instruction_data(99, &PROOF_A, &PROOF_B, &PROOF_C, &PUBLIC_INPUTS);
    let (refuse, _) = executer(data);
    assert!(refuse, "un tag d'instruction inconnu doit être refusé");
}
